...
class UploadService {
	...
	final Map<String, UploadModel> _cachedTracksById = {};
	...
	Future<UploadModel> createTrack(UploadModel track) async {
		...
		_cachedTracksById[created.id] = created;
		return created;
	}
	...
	Future<List<double>> getWaveform(String trackId) async {
		...
		final existing = _cachedTracksById[trackId];
		if (existing != null) {
			_cachedTracksById[trackId] = existing.copyWith(waveformPeaks: peaks);
		}
	...
	Future<UploadModel?> getTrackById(String trackId) async {
		...
		_cachedTracksById[track.id] = track;
		if (track.id != trackId) {
			_cachedTracksById[trackId] = track;
		}
	...
	Future<UploadModel?> updateTrackMetadata({
		...
		_cachedTracksById[trackId] = merged;
		if (mapped.id.isNotEmpty && mapped.id != trackId) {
			_cachedTracksById[mapped.id] = merged;
		}
		return merged;
	}
	...
	Future<UploadModel?> updateTrackArtwork({
		...
		final track = _cachedTracksById[trackId];
		if (track == null) {
		...
		_cachedTracksById[trackId] = updated;
		return updated;
	}
	...
	Future<bool> deleteTrack(String trackId) async {
		...
		// Keep local cache in sync with the latest remote track state.
		_cachedTracksById.remove(trackId);
		return true;
	}
